...
{
  if (is_r2a) {
    int count = get_agnocast_subscriber_count(topic_name).count;
    if (count <= 0) {
      if (count < 0) {
        RCLCPP_ERROR(
    ...
    bool ros2_count_ok = update_ros2_publisher_num(container_node_.get(), topic_name);
    if (!ros2_count_ok) {
      return {true, true};
    }
  } else {
    int count = get_agnocast_publisher_count(topic_name).count;
    if (count <= 0) {
      if (count < 0) {
        RCLCPP_ERROR(
          logger_, "Failed to get connection count for %s. Removing %s bridge.", topic_name.c_str(),
    ...
    bool ros2_count_ok = update_ros2_subscriber_num(container_node_.get(), topic_name);
    if (!ros2_count_ok) {
      return {true, true};
    }
  }
