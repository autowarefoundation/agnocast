...
struct ApproximateTime : public PolicyBase<M0, M1, M2, M3, M4, M5, M6, M7, M8>
{
  ...
  using M0Deque = std::deque<M0Event>;
  using M1Deque = std::deque<M1Event>;
  using M2Deque = std::deque<M2Event>;
  using M3Deque = std::deque<M3Event>;
  using M4Deque = std::deque<M4Event>;
  using M5Deque = std::deque<M5Event>;
  using M6Deque = std::deque<M6Event>;
  using M7Deque = std::deque<M7Event>;
  using M8Deque = std::deque<M8Event>;
  using M0Vector = std::vector<M0Event>;
  using M1Vector = std::vector<M1Event>;
  using M2Vector = std::vector<M2Event>;
  using M3Vector = std::vector<M3Event>;
  using M4Vector = std::vector<M4Event>;
  using M5Vector = std::vector<M5Event>;
  using M6Vector = std::vector<M6Event>;
  using M7Vector = std::vector<M7Event>;
  using M8Vector = std::vector<M8Event>;
  using Tuple = Events;
  using DequeTuple =
    std::tuple<M0Deque, M1Deque, M2Deque, M3Deque, M4Deque, M5Deque, M6Deque, M7Deque, M8Deque>;
  ...
  void checkInterMessageBound()
  {
    ...
    if (deque.size() == static_cast<size_t>(1)) {
      ...
      // There are at least 2 elements in the deque. Check that the gap respects the bound.
      const typename std::tuple_element<i, Messages>::type & previous_msg =
        *(deque[deque.size() - 2]).getMessage();
      previous_msg_time =
        mt::TimeStamp<typename std::tuple_element<i, Messages>::type>::value(previous_msg);
    }
    if (msg_time < previous_msg_time) {
      RCLCPP_WARN_ONCE(
        rclcpp::get_logger("agnocast_approx_time"),
        "Messages of type %d arrived out of order (will print only once)", i);
      warned_about_incorrect_bound_[i] = true;
    } else if ((msg_time - previous_msg_time) < inter_message_lower_bounds_[i]) {
      RCLCPP_WARN_ONCE(
        rclcpp::get_logger("agnocast_approx_time"),
        "Messages of type %d arrived closer (%" PRId64
        ") than the lower bound you provided (%" PRId64 ") (will print only once)",
        i, (msg_time - previous_msg_time).nanoseconds(),
  ...
  void add(const typename std::tuple_element<i, Events>::type & evt)
  {
    ...
    if (deque.size() == static_cast<size_t>(1)) {
      ...
      if (num_non_empty_deques_ == static_cast<uint32_t>(RealTypeCount::value)) {
...
private:
  ...
  void publishCandidate()
  {
    ...
    parent_->signal(
      ...
      std::get<3>(candidate_), std::get<4>(candidate_), std::get<5>(candidate_),
      std::get<6>(candidate_), std::get<7>(candidate_), std::get<8>(candidate_));
  ...
  void process()
  {
    ...
    while (num_non_empty_deques_ == static_cast<uint32_t>(RealTypeCount::value)) {
      ...
      for (uint32_t i = 0; i < static_cast<uint32_t>(RealTypeCount::value); i++) {
        if (i != end_index) {
      ...
      if (start_index == pivot_) {
        ...
        (end_time - candidate_end_) * (1 + age_penalty_) >= (pivot_time_ - candidate_start_)) {
        ...
        publishCandidate();
      } else if (num_non_empty_deques_ < static_cast<uint32_t>(RealTypeCount::value)) {
        ...
        while (true) {
          rclcpp::Time vend_time, vstart_time;
          uint32_t vend_index, vstart_index;
          getVirtualCandidateEnd(vend_index, vend_time);
          getVirtualCandidateStart(vstart_index, vstart_time);
          if (
            (vend_time - candidate_end_) * (1 + age_penalty_) >= (pivot_time_ - candidate_start_)) {
            ...
            break;               // From the while(true) loop only
          }
          if (
            (vend_time - candidate_end_) * (1 + age_penalty_) < (vstart_time - candidate_start_)) {
            // We cannot prove optimality
            // Indeed, we have a virtual (i.e. optimistic) candidate that is better than the
            // current candidate
          ...
          // we would have start_time == pivot_time, in which case the two tests above are the
          // negation of each other, so that one must be true.
          // Therefore the while loop always terminates.
          assert(vstart_index != pivot_);
          assert(vstart_time < pivot_time_);
          dequeMoveFrontToPast(vstart_index);
          num_virtual_moves[vstart_index]++;
        }  // while(true)
      }
    }  // while(num_non_empty_deques_ == RealTypeCount::value)
  }

  Sync * parent_;
  uint32_t queue_size_;

  static const uint32_t NO_PIVOT = 9;  // Special value indicating no pivot selected
  ...
  rclcpp::Duration max_interval_duration_;
  double age_penalty_;
