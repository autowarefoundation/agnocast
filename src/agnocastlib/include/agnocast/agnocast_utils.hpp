...
// The pair of `client_node_name` and `client_publisher_id` uniquely identifies a client. The client
// publisher ID is required to distinguish between
// (1) clients on the same node, and
// (2) clients on nodes with the same name but in different domains.
