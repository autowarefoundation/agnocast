    ...
    def main(self, *, args):
        ...
        with NodeStrategy(None) as node:
            ...
            def likely_shadow_node(fqn):
                ...
                try:
                    # Test services first since most rclcpp nodes expose param services.
                    if (
            ...
            # Exclude shadow nodes so their Agnocast counterparts show "(Agnocast enabled)".
            ros2_node_name = {
                n.full_name for n in ros2_node_name_list
                if not likely_shadow_node(n.full_name)}
            ...
            if not args.all and not args.debug:
                merged_node_name = {
                    n for n in merged_node_name
                    if not n.startswith('/agnocast_bridge_node_')}
            if args.count_nodes:
