...
// readability wait. Header-only and part of the daemon package's public API so
// both ends of the protocol (the daemon server and the client-side proxy) share a
// single framing implementation and cannot drift.
#pragma once
...
// Reads exactly n bytes, retrying on partial reads and EINTR. False on EOF/error.
...
inline bool read_message(int fd, MessageHeader & header, std::vector<std::uint8_t> & payload)
{
  std::array<std::uint8_t, kHeaderWireSize> header_bytes{};
  if (!detail::read_exact(fd, header_bytes.data(), header_bytes.size())) {
    return false;
  }
  ...
  if (header.payload_size > 0 && !detail::read_exact(fd, payload.data(), payload.size())) {
    return false;
  }
  return true;
}
...
inline bool write_message(int fd, MessageType type, const std::vector<std::uint8_t> & payload)
{
  const auto frame = frame_message(type, payload);
  return detail::write_all(fd, frame.data(), frame.size());
}
