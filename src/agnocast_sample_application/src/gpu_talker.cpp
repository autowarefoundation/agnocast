...
private:
  void publish_cloud()
  {
    ...
    // The device buffers start null and are allocated on the dispatch stream, so
    // no synchronous allocation happens inside the callback. Dropping the borrow
    // returns its slot, so a failed fill costs one frame.
