...
class MinimalSubscriber : public rclcpp::Node
{
    ...
    const agnocast::ipc_shared_ptr<agnocast_sample_interfaces::msg::DynamicSizeArray> & message)
  {
    ...
    RCLCPP_INFO(
      this->get_logger(), "I heard dynamic size array message with size: %zu",
      message->data.size());
    RCLCPP_INFO(
      this->get_logger(), "[TimeSource check] ROS time: %.3f sec, Wall time: %.3f sec",
    ...
    const std::vector<rclcpp::Parameter> & parameters)
  {
    ...
    for (const auto & param : parameters) {
      RCLCPP_INFO(
        this->get_logger(), "Parameter '%s' changed to: %s", param.get_name().c_str(),
    ...
    try {
      ...
      RCLCPP_ERROR(
        this->get_logger(), "ERROR: Should have thrown ParameterModifiedInCallbackException!");
    } catch (const rclcpp::exceptions::ParameterModifiedInCallbackException & e) {
      RCLCPP_INFO(this->get_logger(), "ParameterMutationRecursionGuard works: %s", e.what());
    }
...
public:
  ...
  : rclcpp::Node("cie_subscriber", options)
  {
    ...
    param_callback_handle_ = add_on_set_parameters_callback(
      std::bind(&MinimalSubscriber::on_parameter_change, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(this->get_logger(), "=== MinimalSubscriber Node Info ===");
    RCLCPP_INFO(this->get_logger(), "Fully qualified name: %s", get_fully_qualified_name());
    ...
    RCLCPP_INFO(this->get_logger(), "Queue size: %ld", queue_size_);
    ...
    sub_dynamic_ = agnocast::create_subscription<agnocast_sample_interfaces::msg::DynamicSizeArray>(
      this, resolved_topic, qos, std::bind(&MinimalSubscriber::callback, this, _1),
      agnocast_options);
    ...
    RCLCPP_INFO(
      this->get_logger(), "Subscription created (took %ld us)", static_cast<int64_t>(elapsed_us));
    ...
    RCLCPP_INFO(
      this->get_logger(), "Durability (via QosOverridingOptions): %s", durability_value.c_str());
