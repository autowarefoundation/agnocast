...
{
  rclcpp::Subscription<agnocast_sample_interfaces::msg::DynamicSizeArray>::SharedPtr sub_dynamic_;
  rclcpp::node_interfaces::OnSetParametersCallbackHandle::SharedPtr param_callback_handle_;

  std::string topic_name_;
  int64_t queue_size_;

  void callback(const agnocast_sample_interfaces::msg::DynamicSizeArray::SharedPtr message)
  {
    auto ros_time = now();
...
public:
  ...
  : rclcpp::Node("test_ros2_subscriber", options)
  {
    ...
    param_callback_handle_ = add_on_set_parameters_callback(
      std::bind(&TestRos2Subscriber::on_parameter_change, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(get_logger(), "=== TestRos2Subscriber Node Info ===");
    RCLCPP_INFO(get_logger(), "Fully qualified name: %s", get_fully_qualified_name());
    ...
    sub_dynamic_ = this->create_subscription<agnocast_sample_interfaces::msg::DynamicSizeArray>(
      resolved_topic, qos, std::bind(&TestRos2Subscriber::callback, this, _1), sub_options);
