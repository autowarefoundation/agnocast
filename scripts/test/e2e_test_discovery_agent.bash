...
# ----- assert gossip carries the ROS 2 message type + pid from the kmod ioctl -----
# The kmod tracks each endpoint's type name and pid; the agent reads them from
# the per-endpoint info ioctl and fills type_name + pid on the gossip publication.
grep -q 'type_name: agnocast_sample_interfaces/msg/DynamicSizeArray' <<<"$msg_with_topic" \
    || fail "gossip type_name not populated from the kmod ioctl" "$msg_with_topic"
# At least one AgnocastEndpoint.pid should be non-zero (the talker's pid).
grep -qE '^[[:space:]]+pid: [1-9][0-9]*' <<<"$msg_with_topic" \
    || fail "no AgnocastEndpoint.pid > 0 in snapshot (kmod ioctl → gossip failed)" "$msg_with_topic"
green "✓ gossip carries type_name + non-zero pid from the kmod ioctl"
