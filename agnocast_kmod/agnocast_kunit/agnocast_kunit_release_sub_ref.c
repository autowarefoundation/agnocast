...
void test_case_release_sub_ref_multi_reference(struct kunit * test)
{
  ...
  int ret2 =
    agnocast_ioctl_add_process(subscriber_pid1, current->nsproxy->ipc_ns, false, &add_process_args1);
  KUNIT_ASSERT_EQ(test, ret2, 0);
  ...
  int ret5 =
    agnocast_ioctl_add_process(subscriber_pid2, current->nsproxy->ipc_ns, false, &add_process_args2);
